...
require_page_permission('lms_aula_edit.php');
...
if (!$aula_id) {
    redirect('lms_cursos.php', 'Aula não encontrada', 'error');
}
...
$curso_id = $curso_id ?: $aula['curso_id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $url_youtube = $aula['url_youtube'];
    $arquivo_video = $aula['arquivo_video'];
    $arquivo_pdf = $aula['arquivo_pdf'];
    ...
    if ($tipo_conteudo === 'video_youtube') {
        ...
        if (empty($url_youtube)) {
            redirect('lms_aula_edit.php?id=' . $aula_id . '&curso_id=' . $curso_id, 'Preencha a URL do YouTube!', 'error');
        }
        ...
        if (isset($_FILES['arquivo_video']) && $_FILES['arquivo_video']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed)) {
                ...
                if (move_uploaded_file($_FILES['arquivo_video']['tmp_name'], $filepath)) {
                    ...
                    if ($arquivo_video && file_exists(__DIR__ . '/../' . $arquivo_video)) {
                        @unlink(__DIR__ . '/../' . $arquivo_video);
                    }
        ...
        if (empty($conteudo_texto)) {
            redirect('lms_aula_edit.php?id=' . $aula_id . '&curso_id=' . $curso_id, 'Preencha o conteúdo de texto!', 'error');
        }
    }
    
    if (empty($titulo)) {
        redirect('lms_aula_edit.php?id=' . $aula_id . '&curso_id=' . $curso_id, 'Preencha o título da aula!', 'error');
    }
    
    try {
        $stmt = $pdo->prepare("
            UPDATE aulas 
            SET titulo = ?, descricao = ?, ordem = ?, tipo_conteudo = ?, url_youtube = ?, 
                arquivo_video = ?, arquivo_pdf = ?, conteudo_texto = ?, duracao_minutos = ?, 
                duracao_segundos = ?, status = ?
            WHERE id = ?
        ");
        ...
        redirect('lms_aulas.php?curso_id=' . $curso_id, 'Aula atualizada com sucesso!', 'success');
        
    } catch (PDOException $e) {
        error_log("Erro ao atualizar aula: " . $e->getMessage());
        redirect('lms_aula_edit.php?id=' . $aula_id . '&curso_id=' . $curso_id, 'Erro ao atualizar aula. Tente novamente.', 'error');
    }
}

$page_title = 'Editar: ' . $aula['titulo'];
...
<div class="toolbar d-flex flex-stack mb-3 mb-lg-5" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack flex-wrap">
        <div class="page-title d-flex flex-column me-5 py-2">
            <h1 class="d-flex flex-column text-gray-900 fw-bold fs-3 mb-0">Editar Aula</h1>
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
                ...
                <li class="breadcrumb-item text-gray-900">Editar Aula</li>
            </ul>
        </div>
            ...
            <div class="card-body pt-0">
                <form method="POST" enctype="multipart/form-data" class="row g-5">
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="text" name="titulo" class="form-control" required value="<?= htmlspecialchars($aula['titulo']) ?>">
                    </div>
                    
                    <div class="col-md-12">
                        <label class="form-label">Descrição</label>
                        <textarea name="descricao" class="form-control" rows="3"><?= htmlspecialchars($aula['descricao'] ?? '') ?></textarea>
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Ordem</label>
                        <input type="number" name="ordem" class="form-control" value="<?= $aula['ordem'] ?: 0 ?>" min="1">
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Status</label>
                        <select name="status" class="form-select">
                            <option value="rascunho" <?= $aula['status'] == 'rascunho' ? 'selected' : '' ?>>Rascunho</option>
                            <option value="publicado" <?= $aula['status'] == 'publicado' ? 'selected' : '' ?>>Publicado</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="tipo_conteudo" class="form-select" id="tipoConteudo" required>
                            <option value="video_youtube" <?= $aula['tipo_conteudo'] == 'video_youtube' ? 'selected' : '' ?>>Vídeo YouTube</option>
                            <option value="video_upload" <?= $aula['tipo_conteudo'] == 'video_upload' ? 'selected' : '' ?>>Vídeo Upload</option>
                            <option value="pdf" <?= $aula['tipo_conteudo'] == 'pdf' ? 'selected' : '' ?>>PDF</option>
                            <option value="texto" <?= $aula['tipo_conteudo'] == 'texto' ? 'selected' : '' ?>>Texto</option>
                        </select>
                    </div>
                    
                    <!-- Campos por tipo -->
                    <div id="campoVideoYouTube" class="col-md-12" style="display: <?= $aula['tipo_conteudo'] == 'video_youtube' ? 'block' : 'none' ?>;">
                        <label class="form-label">URL do YouTube *</label>
                        <input type="text" name="url_youtube" class="form-control" value="<?= htmlspecialchars($aula['url_youtube'] ?? '') ?>" <?= $aula['tipo_conteudo'] == 'video_youtube' ? 'required' : '' ?>>
                        <div class="form-text">Exemplo: dQw4w9WgXcQ ou https://www.youtube.com/watch?v=dQw4w9WgXcQ</div>
                    </div>
                    
                    <div id="campoVideoUpload" class="col-md-12" style="display: <?= $aula['tipo_conteudo'] == 'video_upload' ? 'block' : 'none' ?>;">
                        <label class="form-label">Arquivo de Vídeo</label>
                        <?php if ($aula['arquivo_video']): ?>
                        <div class="mb-2">
                            <a href="../<?= htmlspecialchars($aula['arquivo_video']) ?>" target="_blank" class="btn btn-sm btn-light">
                        ...
                        <div class="form-text">Deixe em branco para manter o arquivo atual. Formatos: MP4, WEBM, OGG</div>
                    </div>
                    
                    <div id="campoPDF" class="col-md-12" style="display: <?= $aula['tipo_conteudo'] == 'pdf' ? 'block' : 'none' ?>;">
                        <label class="form-label">Arquivo PDF</label>
                        <?php if ($aula['arquivo_pdf']): ?>
                        <div class="mb-2">
                            <a href="../<?= htmlspecialchars($aula['arquivo_pdf']) ?>" target="_blank" class="btn btn-sm btn-light">
                                <i class="ki-duotone ki-file-down fs-4">
                                    <span class="path1"></span>
                                    <span class="path2"></span>
                                </i>
                    ...
                    <div id="campoTexto" class="col-md-12" style="display: <?= $aula['tipo_conteudo'] == 'texto' ? 'block' : 'none' ?>;">
                        <label class="form-label">Conteúdo de Texto *</label>
                        <textarea name="conteudo_texto" class="form-control" rows="10" <?= $aula['tipo_conteudo'] == 'texto' ? 'required' : '' ?>><?= htmlspecialchars($aula['conteudo_texto'] ?? '') ?></textarea>
                        <div class="form-text">Você pode usar HTML para formatar o conteúdo</div>
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Duração (minutos)</label>
                        <input type="number" name="duracao_minutos" class="form-control" min="0" value="<?= $aula['duracao_minutos'] ?? '' ?>">
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Duração (segundos)</label>
                        <input type="number" name="duracao_segundos" class="form-control" min="0" value="<?= $aula['duracao_segundos'] ?? '' ?>">
                    </div>
